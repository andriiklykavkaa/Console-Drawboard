...
public:
    ...
    std::shared_ptr<Command> build(const std::vector<std::string>& tokens) override {
        ...
        std::optional<std::pair<int, int>> coords  = Utils::parse_coords(tokens.at(0), tokens.at(1));
        if (!coords.has_value())
            return nullptr;

        std::shared_ptr<Command> command = std::make_shared<MoveCommand>(coords.value());
        return command;
    }
};
